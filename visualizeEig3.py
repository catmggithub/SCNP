...
if __name__ == '__main__':
    ...
    parser.add_argument('--eig2', type=float, default=0.05)
    ...
    viable_idx = np.argwhere(np.abs(eigs[:,0] - args.eig2) < 0.005)
    viable_idx = viable_idx.reshape(-1)
    viable_idx = np.array(sorted(viable_idx, key=lambda x: eig3[x]))
    ...
    for fig_idx, idx in enumerate(viable_idx):
        ...
        bars_ = axs[fig_idx].bar(range(traj.shape[1]), traj[0], color='#ffca8c', edgecolor='black', linewidth=1.0, label= r'$\lambda_{3}$ ='+ f'{eig3[idx]:.3f}')
        bars.append(bars_)
        axs[fig_idx].set_ylabel('Frequency')
        axs[fig_idx].set_ylim(ymin=0.0, ymax=250.0)
